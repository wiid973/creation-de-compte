...
function Connect-MySQL() { 
    ...
    # Open Connection 
    #uid = nom du compte mysql, pwd = mot de passe
    $connStr = "server=127.0.0.1;port=3306;uid=root;pwd=Admin123456789;database=bdd;Pooling=FALSE" 
    try {
        $conn = New-Object MySql.Data.MySqlClient.MySqlConnection($connStr) 
        $conn.Open()
    } catch [System.Management.Automation.PSArgumentException] {
...
# Exercice 4 : Afficher les informations utilisateur
Write-Host ("--------------------")
$query = "SELECT * FROM Visiteur ORDER BY id DESC LIMIT 5;"
Write-Host ("--------------------")
$result | Format-Table
...
$conn = Connect-MySQL
$query = "insert into visiteur values ( 'd58','prethenvil','widly','widly2000','Admin1234','18 rue etienne dolet','60100','creil','2018-09-01')  ;"
$result = Execute-MySQLQuery $conn $query
Write-Host ("Found " + $result.rows.count + " rows...")